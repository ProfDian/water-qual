...
/**
 * CREATE - Terima data dari ESP32 (EXISTING)
 */
exports.createReading = async (req, res) => {
  try {
    ...
    // 5. TRIGGER FUZZY LOGIC
    try {
      console.log("🔥 Triggering fuzzy logic analysis...");
      const savedDoc = await docRef.get();
      const context = { params: { readingId: docRef.id } };
 ...
 * Body: { ipal_id, sensor_type, sensor_location, sensor_description, status }
 */
exports.addSensor = async (req, res) => {
  try {
    ...
    console.log("➕ Adding new sensor by:", user.email);
    ...
    const validSensorTypes = ["ph", "tds", "turbidity", "temperature"];
    if (!validSensorTypes.includes(sensor_type)) {
      return res.status(400).json({
        success: false,
        message: `Invalid sensor_type. Valid types: ${validSensorTypes.join(
          ", "
        )}`,
      });
    }
